...
DailyRate = st.number_input('DailyRate', min_value=102, max_value=1499, value=802)
DistanceFromHome = st.number_input('DistanceFromHome', min_value=1, max_value=29, value=7)
Education = st.number_input('Education', min_value=1, max_value=5, value=3)
EmployeeNumber = st.number_input('EmployeeNumber', min_value=1, max_value=2068, value=1020)
EnvironmentSatisfaction = st.number_input('EnvironmentSatisfaction', min_value=1, max_value=4, value=3)
HourlyRate = st.number_input('HourlyRate', min_value=30, max_value=100, value=66)
...
MonthlyIncome = st.number_input('MonthlyIncome', min_value=1009, max_value=19999, value=4919)
MonthlyRate = st.number_input('MonthlyRate', min_value=2094, max_value=26999, value=14235)
NumCompaniesWorked = st.number_input('NumCompaniesWorked', min_value=0, max_value=9, value=2)
PercentSalaryHike = st.number_input('PercentSalaryHike', min_value=11, max_value=25, value=14)
PerformanceRating = st.number_input('PerformanceRating', min_value=3, max_value=4, value=3)
...
YearsInCurrentRole = st.number_input('YearsInCurrentRole', min_value=0, max_value=18, value=3)
YearsSinceLastPromotion = st.number_input('YearsSinceLastPromotion', min_value=0, max_value=15, value=1)
YearsWithCurrManager = st.number_input('YearsWithCurrManager', min_value=0, max_value=17, value=3)

OverTime = st.selectbox('OverTime', ('No', 'Yes'))
Gender = st.selectbox('Gender', ('Female', 'Male'))
Department = st.selectbox('Department', ('Sales', 'Research & Development', 'Human Resources'))
BusinessTravel = st.selectbox('BusinessTravel', ('Non-Travel', 'Travel_Rarely', 'Travel_Frequently'))
...
# model expects this column from training feature set
Attrition_0 = 0
...
if st.button('Predict'):
    ...
    if predictions[0] == 1:
        st.error('⚠️High risk of Attrition')
    else:
